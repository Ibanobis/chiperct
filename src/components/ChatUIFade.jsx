...
export default function ChatUIFade() {
  ...
  const handleSubirTexto = async () => {
    if (!nuevoId.trim() || !nuevoTexto.trim()) {
      setConfirmacion("❌ Debes llenar ambos campos.");
      return;
    }

    try {
      const res = await fetch("http://localhost:5000/subir-vector", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: nuevoId, texto: nuevoTexto })
      });
